...
const locSanPham = () => {
    $(".locSanPham").click(function() {
        $(".box-left-form-muaban").addClass("active");
    });
    $(".box-left-form-muaban .close-tab-mb .close-mb").click(function() {
        $(".box-left-form-muaban").removeClass("active");
    });
};
const changeToCurren = () => {
    let ele = $(".wrap-form-calc .wrap-1").find("input");
    var calculateMoney = function() {
        // let soTienVay = $("#sotienvay").val();
        // Ban đầu
        // Máy tính
        let soTienVay = Number(
            $("#sotienvay")
            .val()
            .replace(/[^0-9.-]+/g, "")
        );
        let thoiGianVay = Number($("#thoigianvay").val());
        let laiSuat = Number($("#laisuat").val()) / 100;
        let tienGoc = $("#tienGoc");
        let gocLai = $("#gocLai");
        console.log(laiSuat);
        // Tính lãi cần trả
        let laiCanTra = $("#laiCanTra");
        let lai1nam = soTienVay * laiSuat;
        let lai1thang = lai1nam / 12;
        let TongLai = Math.floor(lai1thang * thoiGianVay);
        // Tiền gốc + lãi
        let calcGocLai = Math.floor(soTienVay / thoiGianVay + TongLai);
        // Kết quả
        tienGoc.text(soTienVay);
        laiCanTra.text(TongLai);
        gocLai.text(calcGocLai);
        $(".money").simpleMoneyFormat();
        $("#tienGoc").simpleMoneyFormat();
        $("#laiCanTra").simpleMoneyFormat();
        $("#gocLai").simpleMoneyFormat();
    };
    ele.on("change keyup", calculateMoney);
};
const checktablink = () => {
...
const scorllToDetail = () => {
    var button = $(".button-sm").outerHeight();
    var thongbao = $(".wrap-list-thongbao").outerHeight();
    if ($(".pages-sanpham").length == 1) {
        if ($(window).width() < 768) {
            $(".top-bar-admin").css("bottom", button);
        }
    }
    $(".wrap-list-thongbao").css("top", thongbao);
};
...
const clickThongBao = () => {
    ...
    $(document).on("click", function(event) {
        var $trigger = $(".nav-item.thong-bao");
        if ($trigger !== event.target && !$trigger.has(event.target).length) {
            $(".wrap-list-thongbao").slideUp();
        }
    });
};
