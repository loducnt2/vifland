                    ...
                    <div
                        ...
                        <div
                            ...
                            <div
                                ...
                                <h1
                                    style="font-size: 30px; color:#0185D0; margin:0; padding-bottom:10px;border-radius: 10px; text-transform: uppercase; ">TIN TỨC BẤT ĐỘNG SẢN </h1>
                                <p style="margin: 0;">Xin chào {{$nguoinhan}},</p>
                                <h2 style="font-size: 24px; padding: 20px; text-align: center; color:#0185D0;">Những thông tin về bất động sản ngày <br>{{$date}}</h2>
                            </div>
                            ...
                            <ul style="padding: 0 20px;">
                                @foreach ($products as $item)
                                    <li style="list-style-type: none; padding-bottom: 70px;">
                                        <div
                                            class="remove-flex-mobile"
                                            style="display:flex; justify-content:space-between;">
                                            <div style="flex-basis: 70%;" class="remove-flex-basis-mobile">

                                        {{-- nhớ sửa đoạn này --}}
                                        {{-- <img src="{{ $message->embed(public_path() . '{{asset('assets/news')}}/{{}}') }}" style="display: block; width: 100%;"> --}}
                                        <img src="{{ $message->embed(public_path() . '/assets/product/detail/'.$item->thumbnail) }} " style="display: block; width: 100%;">

                                    </div>
                                            <div
                                                style="padding-left: 30px; flex-basis: 100%; width: 100%;"
                                                class="remove-flex-basis-mobile">
                                                <h4
                                                    style="font-size: 20px; margin-top: 0; margin-bottom: 10px;"
                                                    class="list-header">{{$item->title}}</h4>
                                                <p style="padding-bottom: 20px; margin-top: 0;">
                                                </p>
                                                <a
                                                    href="vifland.com/article/{{$item->slug}}"
                                                    style="text-decoration: none; text-transform: capitalize; cursor: pointer; line-height: 1.1em; letter-spacing: 0; padding: 12px; background: #0185D0; color: #FFFFFF; border-radius: 5px; text-align: center; font-size: 16px; font-weight: bold; box-sizing: border-box;">Đọc thêm</a>
                                            </div>
                                        </div>
                                    </li>
                                @endforeach
